...
public class TestDepthAng extends SimpleApplication {
	...
	public static void main(String[] args) {
		...
		TestDepthAng app = new TestDepthAng();
		app.start();
	}

	public void simpleInitApp() {
//		Option.findFaceOnlyUpFace=false;
//		Option.enbleMediaFilter=false;
...
//					mosaic.iniAnimation();
	...
	public void updataDepthCam(){
		if (!sceneReader.faceChanged){return;}
		IVec delta=new IVec();
		if (sceneReader.faceChanged){
			IVec normal=sceneReader.getGeo().getDepthCamDir();
			delta=normal.dif(IVec.zaxis);
		}
		IVec dir=Mymath.depthCamDir.sum(delta).unit();
		...
		Mymath.depthCamY=y;
		Mymath.depthCamZ=z;
		Mymath.depthCamDir=dir;
	    Mymath.printIVec(dir);
	}
