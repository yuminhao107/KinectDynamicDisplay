...
public class TestKinect extends SimpleApplication {
	...
	public void simpleInitApp() {
		...
		// drawLine(new IPoint(new IVec(0,0,0),0,0),new IPoint(new
		// IVec(0,0,10),0,0),orange,rootNode);
	...
	public void simpleUpdate(float tpf) {
		...
		if (sceneReader.faceChanged) {
			faces = sceneReader.getFaces();
			rootNode.detachAllChildren();
			IPoint pt = sceneReader.geo.pts[250][200];
			pt.printlnPos();
			// showAllPoints(sceneReader.geo.points());
			this.drawed = true;
			for (IFace face : faces) {
				...
				System.out.print(face.center.z() + " ");
				System.out.println();
			}
		}
	...
	// public void drawShape2(PointGeo geo){
	// Box cube2Mesh = new Box( 1f,1f,1f);
	// Geometry cube2Geo = new Geometry("window frame", cube2Mesh);
	// cube2Geo.setMaterial(orange);
	// Geometry tem;
	// for (IPoint pt:geo.pts){
	// if (!pt.isUseful()) continue;
	// tem=cube2Geo.clone();
	// tem.setLocalTranslation(this.IVecTo3f(pt.pos()));
	// rootNode.attachChild(tem);
	// }
	// drawed=true;
	// }

	// public Node drawShape(PointGeo geo){
	// drawed=true;
	// System.out.println("begin drawing");
	// Node node=new Node();
	//// orange = new Material(assetManager,
	// "Common/MatDefs/Misc/Unshaded.j3md");
	//// orange.setColor("Color", ColorRGBA.Blue);
	// ArrayList<IPoint> stack = new ArrayList<IPoint>();
	// stack.add(geo.origin);
	// while (!stack.isEmpty()) {
	//
	...
	// if (pt.down()!=null){
	// if (pt.down().isUseful()){
	// drawLine(pt,pt.down(),orange,node);
	// }
	// }
	// if (pt.right()!=null){
	// if (pt.right().isUseful()){
	// drawLine(pt,pt.right(),orange,node);
	// }
	// }
	// }
	// if (num>100) return node;
	//
	// if (pt.right() != null)
